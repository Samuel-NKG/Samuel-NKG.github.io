document.addEventListener('DOMContentLoaded', function () {
          ...
          <div class="research-right">
            <div class="research-panel active" data-index="0">
              ...
              <figure class="research-figure">
                <img src="/img/few-shot-overview.jpg" alt="Few-shot learning overview">
                <figcaption>Image adapted from common few-shot learning illustrations.</figcaption>
              </figure>
              <h3>My Research</h3>
              <p>Building upon these foundations, a notable challenge emerges when adapting large-scale vision-language models such as CLIP to few-shot regimes: the frequent entanglement of foreground objects with background context within the extracted visual features.</p>
              <p>One effective direction is to explicitly decouple this confusion through complementary modeling. Hierarchical queries can progressively separate multi-level visual features into distinct foreground and background streams.</p>
            </div>
            <div class="research-panel" data-index="1">
              <h3>Satellite Video Tracking</h3>
              <p>Satellite video tracking is a specialized form of single-object tracking that continuously estimates the state of moving targets from overhead satellite sequences after initialization in the first frame.</p>
              <p>One effective direction is to model historical target appearance as a compact low-rank manifold-structured subspace rather than storing raw template features.</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="research-card page-card" id="projects-card">
          ...
          <div class="research-right">
            <div class="research-panel active" data-project="0">
              ...
              <p>Forest is a blockchain-based privacy-preserving platform designed to enable secure, transparent, and efficient data collaboration while rigorously protecting data ownership and privacy.</p>
            </div>
            <div class="research-panel" data-project="1">
              <h3>Qin Mountains Four Treasures Detection System Based on YOLO and ESP32-CAM</h3>
              <p>An intelligent wildlife monitoring platform for the protection of four iconic species native to the Qinling Mountains.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  function typeInto(el, text, cursor, done) {
    ...
    function tick() {
      if (i <= text.length) {
        ...
        if (cursor) { cursor.classList.remove('is-typing'); cursor.style.opacity = '0'; }
        if (done) done();
      }
    }
    tick();
  ...
  function updateByScroll() {
    ...
    pages.forEach(function (page) {
      ...
      if (!played[page.id] && rect.top < window.innerHeight * 0.55 && pageRect.bottom > window.innerHeight * 0.25) {
        playPage(page);
      }
      if (played[page.id]) {
